...
class APISendMailReminders(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        pending_stages = Document.objects.raw(f"""SELECT nested.workflow as wx, nested.label, nested.state_id as state_id, nested.state_id as id, count(nested.state_id) as ecount, nts.email
            FROM (
                SELECT DISTINCT ON(id) swi.id AS id, swi.workflow_id AS workflow, wf.label, sws.id AS state_id, max(swil.datetime) AS datetime
                FROM document_states_workflowinstance swi, document_states_workflowinstancelogentry swil,
                document_states_workflowtransition swt, document_states_workflowstate sws, document_states_workflow wf
            ...
            group by nts.email, nested.state_id, nested.workflow, nested.label

            UNION

            SELECT max(9) AS wx, wf.label, max(66) AS state_id, max(swi.document_id) AS id, count(md.value) as ecount, md.value AS email
